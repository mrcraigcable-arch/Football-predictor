...
st.set_page_config(page_title="Craig's Football Predictor V19.1", page_icon="📈", layout="wide")
...
<div class="v14-brand">
 <span class="v14-chip">V19.1</span>
...
<div class="brand">
  ...
  <div><div class="brand-name">Craig's Football Predictor <span class="vbadge">V19.1</span></div>
...
if True:
    ...
    def _build_goal_acca(frame,legs,stake,target):
        pool=frame[frame["Pick"].isin(["HOME","AWAY"])].copy()
        pool["_goal_price"]=pool.apply(_goal_price,axis=1)
        pool["_goal_prob"]=pd.to_numeric(pool["Confidence %"],errors="coerce")/100.0
        pool["_books"]=pd.to_numeric(pool["Bookmakers"],errors="coerce")
        pool=pool[(pool["_goal_price"]>1.0) & (pool["_goal_prob"]>0) &
                  pool["Market integrity"].eq("OK") & pool["Kickoff ISO"].notna() &
                  (pool["_books"]>=float(min_books))]
        pool=pool.sort_values("Confidence %",ascending=False).head(24).reset_index(drop=True)
        if len(pool)<legs:
            return None,{"reason":f"Only {len(pool)} win picks currently have a verified price, verified kickoff, market-integrity pass and at least {min_books} bookmakers. {legs} are required."}
        target_odds=float(target)/float(stake) if float(stake)>0 else np.inf
        all_combos=[]
        for idxs in combinations(range(len(pool)),int(legs)):
            part=pool.iloc[list(idxs)]
            total_odds=float(np.prod(part["_goal_price"].to_numpy(float)))
            joint_prob=float(np.prod(part["_goal_prob"].to_numpy(float)))
            ret=float(stake)*total_odds
            all_combos.append((idxs,total_odds,joint_prob,ret))
        if not all_combos:
            return None,{"reason":"No valid accumulator combinations were available."}
        qualifying=[x for x in all_combos if x[1]>=target_odds]
        if qualifying:
            # Primary objective: highest model chance among combinations that reach the target.
            best=max(qualifying,key=lambda x:(x[2],-abs(x[1]-target_odds)))
            status="TARGET REACHED"
        else:
            # Honest fallback: closest achievable return below the target; chance breaks ties.
            best=max(all_combos,key=lambda x:(x[1],x[2]))
            status="TARGET NOT REACHABLE"
        part=pool.iloc[list(best[0])].copy()
        return {"legs":part,"total_odds":best[1],"joint_prob":best[2],"return":best[3],"target_odds":target_odds,"status":status},None
...
st.divider()
st.caption("V19.1 rule: BET requires matched current UK 1X2 prices, verified fixture/kickoff, bookmaker depth, confidence, minimum edge and positive EV. Live validation records evidence; it does not loosen betting rules.")
